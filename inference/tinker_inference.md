...
- `evaluate_harness1.py` has `temperature=1.0` as its default, while the
  historical `eval_sft_ultra_0417.py` file has an older `temperature=0.6`
  default. Always pass `--temperature 1.0` explicitly for Table 2 reproduction.
- `MAX_TURNS` defaults to `35` in `harness.ultra_core`, but the Table 2
  HarnesS-1 run used `--max-turns 40`. Always pass it explicitly.
- The public Tinker checkpoint path is sufficient for Tinker inference once the
  checkpoint has been published by the owner.
